...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut token_expr =
        quote! { dharitri_sc::types::EgldOrEsdtTokenIdentifier::<Self::Api>::egld() };
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            ...
            contract_call_type = quote! { dharitri_sc::types::ContractCallWithEgldOrSingleEsdt };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_sc::types::ContractCallWithEgldOrSingleEsdt::new(
                    ___address___,
                    #endpoint_name,
                    #token_expr,
                    #nonce_expr,
        ...
        contract_call_type = quote! { dharitri_sc::types::ContractCallWithMultiEsdt };
        contract_call_init = quote! {
            let mut ___contract_call___ = dharitri_sc::types::ContractCallWithMultiEsdt::new(
                ___address___,
                #endpoint_name,
